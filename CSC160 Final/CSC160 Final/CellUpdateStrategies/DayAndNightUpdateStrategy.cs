...
{
    public class DayAndNightUpdateStrategy : ICellUpdateStrategy
    {
        public void Update(List<List<Cell>> cells, Grid gameGrid)
        {
            ...
            foreach (List<Cell> row in cells)
            {
                foreach (Cell cell in row)
                {
                    ...
                    if (amountAlive < 3 || amountAlive == 5) cell.IsAlive = false;
                    else if (!cell.IsAlive && amountAlive is 3 or 6 or 7 or 8) cell.IsAlive = true;
                }
            }
        }
